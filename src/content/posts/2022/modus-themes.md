...
用过很多Emacs主题，做得不错的主题有 `doom-themes` ，[ef-themes](https://elpa.gnu.org/packages/ef-themes.html)。其中 `doom-themes` 里面有很多不错的配色，比如Material，Gruvbox，Miramare。不过适配最广的还是[modus-themes](https://elpa.gnu.org/packages/modus-themes.html)。
...
`modus-themes` 需要Emacs版本最低为27.1。在Emacs 28.1的版本已经自带了 `modus-themes` 主题，而27.1版本可以通过package.el的elpa安装[^1]，在Debian里面也有一个[elpa-modus-themes](https://packages.debian.org/stable/elpa-modus-themes)包，可以用自带的包管理器安装。
